...
class TestOnEventPlumbing(unittest.TestCase):
    ...
    def make_client(self, endpoint_name: str, endpoint_config: dict[str, Any], *, on_event=None) -> APIClient:
        schema: dict[str, Any] = {
    ...
    def test_on_event_exceptions_are_swallowed(self):
        ...
        client = self.make_client(
            'list_events',
            {
                ...
                'pagination': cursor_pagination(cursor_param='cursor', next_cursor_path='meta.next_cursor', data_path='items'),
            },
            on_event=boom,
        )
    ...
    def test_rate_limit_validation_rejects_non_positive_rps(self):
        with TemporaryDirectory() as td:
            ...
            with self.assertRaises(SchemaError):
                APIClient({
                    'base_url': 'https://example.invalid',
                    'rate_limit': {'strategy': 'token_bucket', 'requests_per_second': 0, 'burst': 1, 'on_limit': 'wait'},
                    'endpoints': {'t': {'method': 'GET', 'path': '/t', 'response_format': 'json', 'playback': {'mode': 'load', 'path': str(fixture)}}},
                })
    ...
    def test_rate_limit_validation_rejects_non_positive_burst(self):
        with TemporaryDirectory() as td:
            ...
            with self.assertRaises(SchemaError):
                APIClient({
                    'base_url': 'https://example.invalid',
                    'rate_limit': {'strategy': 'token_bucket', 'requests_per_second': 1.0, 'burst': 0, 'on_limit': 'wait'},
                    'endpoints': {'t': {'method': 'GET', 'path': '/t', 'response_format': 'json', 'playback': {'mode': 'load', 'path': str(fixture)}}},
                })
    ...
    def test_rate_limit_bypassed_in_playback(self):
        ...
        with TemporaryDirectory() as td:
            ...
            client = APIClient({
                'base_url': 'https://example.invalid',
                'on_event': on_event,
                'rate_limit': {'strategy': 'token_bucket', 'requests_per_second': 0.0001, 'burst': 1, 'on_limit': 'raise'},
                'endpoints': {'t': {'method': 'GET', 'path': '/t', 'response_format': 'json', 'playback': {'mode': 'load', 'path': str(fixture)}}},
            })
...
class TestRateLimitEventReporting(unittest.TestCase):
    ...
    def test_retry_wait_end_event_includes_reactive_wait_fields(self):
        ...
        with patch('requests.Session.request', side_effect=[resp1, resp2]), patch('time.sleep', return_value=None):
            client = APIClient({
                'base_url': 'https://example.invalid',
                'on_event': lambda ev: events.append(ev),
                'retry': {'max_attempts': 2, 'backoff': 'linear', 'base_delay': 0.001, 'max_delay': 0.001, 'on_codes': [429], 'max_retry_after': 10.0},
                'rate_limit': {'respect_retry_after': True},
                'endpoints': {'t': {'method': 'GET', 'path': '/t', 'response_format': 'json'}},
            })
            result = client.fetch('t')
        self.assertEqual(result, {'ok': True})
        reactive_wait = next(e for e in events if e.kind == 'rate_limit_wait_end' and e.data.get('wait_type') == 'reactive')
    ...
    def test_non_rate_limit_request_error_still_routes_through_on_error(self):
        ...
        with TemporaryDirectory() as td:
            ...
            fixture.write_text(json.dumps([envelope], ensure_ascii=False, indent=2), encoding='utf-8')
...
class TestRateLimitOverrideSemantics(unittest.TestCase):
    def test_endpoint_rate_limit_replaces_client_retry_after_policy(self):
        sleeps: list[float] = []
        responses = [
            MagicMock(status_code=429, ok=False, headers={'Retry-After': '5'}, url='https://example.invalid/t'),
        ...
        client = APIClient({
            'base_url': 'https://example.invalid',
            'retry': {'max_attempts': 2, 'backoff': 'linear', 'base_delay': 7.0, 'max_delay': 7.0, 'max_retry_after': 30.0},
            'rate_limit': {'respect_retry_after': True},
            'endpoints': {
                't': {
                    'method': 'GET',
        ...
        with patch('requests.Session.request', side_effect=responses),              patch('time.sleep', side_effect=lambda s: sleeps.append(s)):
            result = client.fetch('t')
    ...
    def test_endpoint_rate_limit_replacement_controls_max_retry_after(self):
        response = MagicMock(status_code=429, ok=False, headers={'Retry-After': '5'}, url='https://example.invalid/t')
        response.json.return_value = {'err': True}
        response.text = '{"err": true}'

        client = APIClient({
            'base_url': 'https://example.invalid',
            'retry': {'max_attempts': 2, 'backoff': 'linear', 'base_delay': 0.1, 'max_delay': 0.1, 'max_retry_after': 30.0},
            'rate_limit': {'respect_retry_after': True},
            'endpoints': {
                't': {
                    'method': 'GET',
...
class TestRetryEventPayloads(unittest.TestCase):
    def test_status_code_retry_reports_planned_ms_from_retry_after(self):
        ...
        client = APIClient({
            'base_url': 'https://example.invalid',
            'on_event': on_event,
            'retry': {'max_attempts': 2, 'backoff': 'linear', 'base_delay': 1.0, 'max_delay': 1.0},
            'rate_limit': {'respect_retry_after': True},
            'endpoints': {'t': {'method': 'GET', 'path': '/t', 'response_format': 'json'}},
        })
    ...
    def test_retry_event_planned_ms_is_consistent_across_status_and_network_paths(self):
        ...
        status_client = APIClient({
            ...
            'retry': {'max_attempts': 2, 'backoff': 'linear', 'base_delay': 2.0, 'max_delay': 2.0},
            'rate_limit': {'respect_retry_after': False},
            'endpoints': {'t': {'method': 'GET', 'path': '/t', 'response_format': 'json'}},
        })
        ...
        with patch('requests.Session.request', side_effect=[_json_response({'err': True}, status=500), _json_response({'ok': True}, status=200)]), patch('time.sleep'):
            self.assertEqual(status_client.fetch('t'), {'ok': True})
        with patch('requests.Session.request', side_effect=[requests.ConnectionError('down'), _json_response({'ok': True}, status=200)]), patch('time.sleep'):
...
class TestG3WaitObservability(unittest.TestCase):
    def test_reactive_wait_end_includes_planned_ms_and_cause(self):
        ...
        with patch('requests.Session.request', side_effect=[resp1, resp2]), patch('time.sleep', return_value=None):
            client = APIClient({
                'base_url': 'https://example.invalid',
                'on_event': lambda ev: events.append(ev),
                'retry': {'max_attempts': 2, 'backoff': 'linear', 'base_delay': 0.001, 'max_delay': 0.001, 'on_codes': [429]},
                'rate_limit': {'respect_retry_after': True},
                'endpoints': {'t': {'method': 'GET', 'path': '/t', 'response_format': 'json'}},
            })
            client.fetch('t')
        retry_event = next(e for e in events if e.kind == 'retry')
        wait_end = next(e for e in events if e.kind == 'rate_limit_wait_end' and e.data.get('wait_type') == 'reactive')
    ...
    def test_proactive_wait_end_planned_ms_matches_start(self):
        ...
        with patch('rest_fetcher.client.TokenBucket', return_value=FakeBucket()),              patch('time.monotonic', side_effect=[1.0, 2.0, 2.4, 3.0]):
            client = APIClient({
                'base_url': 'https://example.invalid',
                'on_event': lambda ev: events.append(ev),
                'rate_limit': {'strategy': 'token_bucket', 'requests_per_second': 1.0, 'burst': 1},
                'endpoints': {'t': {'method': 'GET', 'path': '/t', 'response_format': 'json'}},
            })
    ...
    def test_terminal_retry_none_respects_max_retry_after(self):
        ...
        response.ok = False
        with patch('requests.Session.request', return_value=response), patch('time.sleep', return_value=None):
            client = APIClient({
                'base_url': 'https://example.invalid',
                'on_event': lambda ev: events.append(ev),
                'retry': {'max_attempts': 2, 'max_retry_after': 1.0},
                'rate_limit': {'respect_retry_after': True},
                'endpoints': {'t': {'method': 'GET', 'path': '/t', 'response_format': 'json', 'retry': None}},
            })
            with self.assertRaises(RateLimitError):
                client.fetch('t')
        self.assertFalse(any(e.kind == 'rate_limit_wait_end' and e.data.get('wait_type') == 'reactive' for e in events))
    ...
    def test_stream_summary_splits_retry_and_rate_limit_waits(self):
        ...
        with patch('requests.Session.request', side_effect=[requests.ConnectionError('boom'), good]), patch('time.sleep', return_value=None):
            client = APIClient({
                'base_url': 'https://example.invalid',
                'retry': {'max_attempts': 2, 'backoff': 'linear', 'base_delay': 0.25, 'max_delay': 0.25},
                'endpoints': {'t': {'method': 'GET', 'path': '/t', 'response_format': 'json', 'pagination': {'next_request': lambda parsed_body, state: None}, 'on_complete': on_complete}},
            })
...
class TestBytesReceivedTelemetry(unittest.TestCase):
        ...
        with patch('requests.Session.request') as mock_req:
            ...
            client = APIClient({
                ...
                'endpoints': {'ep': {
                    ...
                    'pagination': {
                        'next_request': lambda resp, state: {'params': {'p': 2}} if resp.get('next') else None,
                    },
    ...
    def test_retryable_exhaustion_reports_bytes_and_status(self):
        ...
        with patch('requests.Session.request') as mock_req:
            ...
            client = APIClient({
                'base_url': 'https://api.example.com/v1',
                'retry': {'max_attempts': 2, 'backoff': 'linear', 'base_delay': 0.001, 'max_delay': 0.001, 'on_codes': [500]},
                'on_event': lambda e: events.append(e),
                'on_error': lambda exc, state: 'skip',
                'endpoints': {'ep': {'method': 'GET', 'path': '/items'}},
            })
    ...
    def test_playback_contributes_bytes(self):
        ...
        import os
        body = {'items': [1, 2]}
        fixture = [{'kind': 'raw_response', 'format': 'json', 'status_code': 200,
                     'headers': {'Content-Type': 'application/json'},
...
class TestRetryBytesReceivedTelemetry(unittest.TestCase):
    ...
    def test_no_retry_summary_and_event_are_zero(self):
        ...
        with patch('requests.Session.request') as mock_req:
            ...
            client = APIClient({
                'base_url': 'https://api.example.com/v1',
                'on_event': lambda e: events.append(e),
                'endpoints': {'ep': {'method': 'GET', 'path': '/items', 'on_complete': on_complete}},
            })
            list(client.stream('ep'))
    ...
    def test_retry_exhaustion_sums_retry_attempt_bytes_including_final_error(self):
        ...
        with patch('requests.Session.request') as mock_req:
            ...
            client = APIClient({
                'base_url': 'https://api.example.com/v1',
                'retry': {'max_attempts': 3, 'backoff': 'linear', 'base_delay': 0.001, 'max_delay': 0.001, 'on_codes': [500]},
                'on_event': lambda e: events.append(e),
                'on_error': lambda exc, state: 'skip',
                'endpoints': {'ep': {'method': 'GET', 'path': '/items'}},
            })
    ...
    def test_request_end_retry_bytes_received_is_per_cycle_not_per_run(self):
        ...
        with patch('requests.Session.request') as mock_req:
            ...
            client = APIClient({
                ...
                'retry': {'max_attempts': 2, 'backoff': 'linear', 'base_delay': 0.001, 'max_delay': 0.001, 'on_codes': [500]},
                'on_event': lambda e: events.append(e),
                'endpoints': {'ep': {'method': 'GET', 'path': '/items', 'pagination': {'next_request': next_request}}},
            })
            with patch('time.sleep', return_value=None):
                client.fetch('ep')
