...
class TestRecordedResponse(unittest.TestCase):
    ...
    def test_headers_are_case_insensitive(self):
        ...
        rec = RecordedResponse(
            200, CaseInsensitiveDict({'content-type': 'application/json'}), 'json', '', '{}'
        )
...
class TestRawPlaybackRoundTrip(unittest.TestCase):
    ...
    def test_single_page_save_and_load_json(self):
        ...
        with patch('requests.Session.request') as mock_req:
            mock_req.return_value = make_response(
                200, {'result': 42}, headers={'Content-Type': 'application/json'}
            )
            client = APIClient(
                {
    ...
    def test_on_response_can_be_changed_on_load_for_debugging(self):
        ...
        with patch('requests.Session.request') as mock_req:
            mock_req.return_value = make_response(
                200, {'x': 10, 'y': 20}, headers={'Content-Type': 'application/json'}
            )
            client = APIClient(
                {
                    'base_url': 'https://api.example.com',
    ...
    def test_response_parser_runs_during_load(self):
        ...
        with patch('requests.Session.request') as mock_req:
            mock_req.return_value = make_response(
                200, {'a': 1}, headers={'Content-Type': 'application/json'}
            )
            client = APIClient(
                {
                    'base_url': 'https://api.example.com',
    ...
    def test_non_paginated_live_endpoint_runs_on_page_and_on_complete(self):
        ...
        with patch('requests.Session.request') as mock_req:
            mock_req.return_value = make_response(
                200, {'x': 7}, headers={'Content-Type': 'application/json'}
            )
            client = APIClient(
                {
                    'base_url': 'https://api.example.com',
    ...
    def test_fixture_written_when_stream_is_broken_early(self):
        ...
        responses = [
            make_response(
                200, {'items': [i], 'next': i < 5}, headers={'Content-Type': 'application/json'}
            )
            for i in range(1, 6)
        ]
        ...
        with open(path, encoding='utf-8') as f:
            payload = json.load(f)
        self.assertEqual(
            len(payload), 2, 'fixture should contain exactly the 2 pages that were fetched'
        )
    ...
    def test_non_paginated_save_with_on_complete_writes_fixture(self):
        ...
        self.assertTrue(
            os.path.exists(path),
            'fixture file must exist after non-paginated save with on_complete',
        )
        with open(path, encoding='utf-8') as f:
            payload = json.load(f)
    ...
    def test_paginated_fetch_with_on_complete_writes_fixture(self):
        ...
        r1 = make_response(
            200, {'items': [1, 2], 'next': True}, headers={'Content-Type': 'application/json'}
        )
        r1.url = 'https://api.example.com/x?page=1'
        r2 = make_response(
            200, {'items': [3], 'next': False}, headers={'Content-Type': 'application/json'}
        )
        ...
        self.assertTrue(
            os.path.exists(path), 'fixture file must exist after paginated fetch with on_complete'
        )
        with open(path, encoding='utf-8') as f:
            payload = json.load(f)
    ...
    def test_fixture_records_raw_response_envelope(self):
        ...
        with patch('requests.Session.request') as mock_req:
            mock_req.return_value = make_response(
                200, {'k': 'v'}, headers={'Content-Type': 'application/json'}
            )
            client = APIClient(
                {
                    'base_url': 'https://api.example.com',
...
class TestPlaybackScrubbing(unittest.TestCase):
    ...
    def test_paginated_stream_writes_raw_playback_fixture(self):
        ...
        r1 = make_response(
            200,
            {'items': [{'id': 1}, {'id': 2}], 'meta': {'total': 3}},
            headers={'Content-Type': 'application/json'},
        )
        r1.url = 'https://api.example.com/items?page=1'
        r2 = make_response(
            200,
            {'items': [{'id': 3}], 'meta': {'total': 3}},
            headers={'Content-Type': 'application/json'},
        )
    ...
    def test_raw_playback_saves_unscrubbed_response_headers_and_body(self):
        ...
        r = make_response(
            200,
            {'token_echo': 'abc123'},
            headers={'Content-Type': 'application/json', 'X-Response-Token': 'resp-secret'},
        )
        r.url = 'https://api.example.com/items?tenant_token=abc123&page=1'
        with patch('requests.Session.request', return_value=r):
            client = APIClient(schema)
    ...
    def test_playback_mode_none_disables_playback_without_path(self):
        ...
        with patch(
            ...
            return_value=make_response(
                200, [{'id': 1}], headers={'Content-Type': 'application/json'}
            ),
        ):
...
class TestPlaybackFixtureFormat(unittest.TestCase):
    ...
    def test_recording_textual_fixture_fails_on_undecodable_bytes(self):
        ...
        with self.assertRaises(FixtureFormatError):
            _serialize_playback(
                resp, response_format='text', encoding='utf-8', record_as_bytes=False
            )
...
class TestPlaybackLoadBehavior(unittest.TestCase):
            ...
            client = APIClient(
                {
                    ...
                    'endpoints': {
                        'x': {
                            ...
                            'playback': {'path': playback_file, 'mode': 'load'},
                            'pagination': {
                                'next_request': lambda resp, state: {'params': {'page': 2}}
                                if resp.get('next')
                                else None,
                            },
    ...
    def test_playback_save_records_raw_json_response(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('requests.Session.request') as mock_req:
                mock_req.return_value = make_response(
                    200, {'items': [1]}, headers={'Content-Type': 'application/json'}
                )
                client = APIClient(
                    {
                        'base_url': 'https://api.example.com',
