...
class CallbackError(RestFetcherError):
    ...
    def __init__(
        self, message: str, callback_name: str | None = None, cause: BaseException | None = None
    ) -> None:
        super().__init__(message)
...
class DeadlineExceeded(RestFetcherError):
    ...
    def __init__(self, limit: float, elapsed: float) -> None:
        super().__init__(f'time_limit exceeded: {elapsed:.2f}s elapsed, limit was {limit:.2f}s')
        self.limit = limit
        self.elapsed = elapsed


def raise_(
    exc_or_msg: str | BaseException, cls: type[RestFetcherError] = ResponseError, **kwargs: Any
) -> None:
    """
