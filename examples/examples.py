...
if run.summary is not None:
    ...
    if run.summary.stop:
        print(f"stopped by {run.summary.stop.kind}: {run.summary.stop.observed}/{run.summary.stop.limit}")
...
all_rows = client.fetch('report', params={
    'ids': '12345678',
    'metrics': 'ym:s:visits',
    'date1': '2025-01-01',
    'date2': '2025-01-31',
...
try:
    all_rows = client.fetch('report',
        params={'ids': '12345678', 'metrics': 'ym:s:visits',
    ...
    'endpoints': {
        'data': {
            ...
            'on_response': lambda resp, state: (
                ...
                if 'error' in resp else resp['data']
            ),
        }
    }
})
...
client = APIClient({
    ...
    'endpoints': {
        'records': {'method': 'GET', 'path': '/records'}
    }
})
...
def get_token_from_state(config):
    ...
    return boto3.client('ssm').get_parameter(
        Name=config['ssm_param_name'], WithDecryption=True
    )['Parameter']['Value']
...
client = APIClient({
    ...
    'endpoints': {
        'events': {
            ...
            'on_complete': lambda pages, state: {  # fetch() materializes all pages before completion
    ...
    'endpoints': {
        'reports': {
            ...
            'pagination': {
                ...
                'initial_params': {'page': 1, 'per_page': 50},  # sent on first request automatically
            },
...
reports = client.fetch('reports', params={'per_page': 100})   # initial_params overridden at call time
...
client = APIClient({
    ...
    'endpoints': {
        ...
        'users': {
            'method': 'GET',
            'path': '/users',
            'playback': {'path': 'examples/fixtures/example_26_playback_users.json', 'mode': 'load'},
        },
...
schema = (
    ...
    .pagination({                                           # inherited by all endpoints
    ...
    .endpoint('webhook', method='POST', path='/webhooks',
              pagination=None)                              # disable for this endpoint
    .build()
)
...
def stop_after_three_errors(exc, state):
    ...
    if errors_so_far >= 3:
        print(
            f'stopping after {errors_so_far} errors '
            f'with {pages_so_far} pages already collected'
        )
        return 'stop'
...
client = APIClient({
    ...
    'endpoints': {
        'paged': {
            ...
            'pagination': page_number_pagination(data_path='items', total_pages_path='total_pages'),
        }
    }
})
...
def on_stream_complete(summary, state):
    if summary.stop is not None:
        print(f"stream stopped by {summary.stop.kind} at {summary.stop.observed} / {summary.stop.limit}")
    else:
        print(f"stream completed naturally after {summary.pages} pages and {summary.requests} requests")
...
if summary is not None:
    print(summary.endpoint, summary.source, summary.requests, summary.retries, summary.elapsed_seconds)
    if summary.stop is not None:
...
def pace_after_slow_page(outcome: PageCycleOutcome, state: dict) -> float | None:
    if outcome.kind == 'success' and outcome.cycle_elapsed_ms is not None and outcome.cycle_elapsed_ms > 2000:
        return 0.5
    return None
...
def log_stop_reason(outcome: PageCycleOutcome, state: dict) -> float | None:
    if outcome.kind == 'stopped' and outcome.stop_signal is not None:
        print(f'run stopped: kind={outcome.stop_signal.kind} '
